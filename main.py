...
BUTTONS = {"start": "Начать переписку"}
RESULTS_CHECK = {"High": "высокая", "Low": "низкая"}
# Логи
logging.basicConfig(
    level=logging.DEBUG,
    filename="log_bot",
...
def start(update, context):
    keyboard = [[InlineKeyboardButton(BUTTONS["start"], callback_data="start")]]
...
def send_request_to_api_web(input_data: str) -> List[str]:
    ...
    if response.status_code != HTTPStatus.OK:
        error_text = "Ошибочный ответ от сервиса " f"Гардиум: {response.status_code}"
        logger.error(error_text)
        raise ValueError(error_text)
...
def handle_message(update, context):
    ...
    if message_text in MESSAGES:
    ...
    elif message_text not in MESSAGES and re.match(r"[\w\d_\-=.]{1,30}", message_text):
        logger.debug("Пользователь направил запрос, соответствующий параметрам.")
        ...
        logger.debug(f"Вычислена вероятность регистрации названия: {complexity}.")
        logger.debug(f"Сгенерирована ссылка на полный отчет: {url_report}.")
...
def button(update, context):
    ...
    query.edit_message_text(
        text=(
            "Введите название, которое вы хотели бы проверить."
...
def send_start_button(context):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    context.bot.send_message(
        chat_id=TELEGRAM_CHAT_ID, text=MESSAGES["start"], reply_markup=reply_markup
    )
